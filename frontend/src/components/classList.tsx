...
import { api, _Class, Course } from '../lib/api';   
...
function CourseRow({ scheduleData }: { scheduleData: Course }) {
  // Grab the readable course title, fallback gracefully if data is missing
  const courseTitle = scheduleData.courseTitle || "Untitled Course";
  const courseCode = scheduleData.courseCode || "N/A";
  
  const enrollStudent = async (cid: string | undefined) => {
    api.post("/students/enroll", {
      cid: cid
    })
  }
  ...
  return (
    ...
    <div className="flex flex-col w-5/6 sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-slate-100 hover:border-slate-200 transition-colors">
      ...
      <div className="flex items-center gap-4 w-full sm:w-72">
        ...
        <div>
          <h4 className="text-base font-semibold text-slate-900 leading-tight">{courseTitle}</h4>
          <span className="text-xs font-mono text-slate-400">CourseCode: {courseCode}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col min-w-[150px]">
        <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Professor</span>
        <span className="text-sm font-medium text-slate-700">{scheduleData.description?.substring(0,5) || "No Mentor Assigned"}</span>
      </div>

      {/* Schedule Time Indicators */}
      <div className="flex flex-col min-w-[120px]">
        <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Schedule</span>
        <span className="text-sm font-semibold text-slate-800">{scheduleData.credits}</span>
        {scheduleData._id && <span className="text-xs text-slate-500">{scheduleData._id}</span>}
      </div>
      
    </div>
    <div className='flex  w-1/6 justify-center sm:items-center sm:flex-row rounded-xl '>
      <button onClick={() => enrollStudent(scheduleData._id)} className="font-semibold text-white font-serif bg-accent-color rounded-xl p-3 shadow-sm border border-slate-100 hover:border-slate-200 transition-colors"> Enroll</button>
    </div>
    </div>
  );
}
...
export default function TechTownDashboard() {
  ...
  const renderComp = (nut: any, classes: _Class[], courses: Course[]) => {
    switch(nut) {
      case 0:
    ...
    <div className="flex min-h-screen bg-slate-50 font-sans antialiased text-slate-800">
      ...
      <aside className="hidden md:flex flex-col w-60 bg-gradient-to-b from-red-600 to-red-800 text-white p-6 shadow-xl">
        ...
        <nav className="flex flex-col gap-1">
          <div onClick={() => handleClick(0)} className="p-3 rounded-lg text-sm font-semibold bg-slate-900 text-white shadow-inner cursor-pointer">
            My Classes
          </div>
          <div onClick={() => handleClick(1)} className="p-3 rounded-lg text-sm font-medium text-red-100 hover:bg-red-700 transition-colors cursor-pointer">
            Enrolled Courses
          </div>
          <div onClick={() => handleClick(2)} className="p-3 rounded-lg text-sm font-medium text-red-100 hover:bg-red-700 transition-colors cursor-pointer">
            Campus Venues
          </div>
        </nav>
      </aside>
